<div class="product ">
    <div class="sort_overlay"></div>
    <div class="product_title pad">
        <div class="product_title-h1">
            <h1>New</h1>
        </div>
        <div class="product_title-p">
            <p>Khám phá aether house</p>
        </div>
    </div>
    <div class="product_filter">
        <div class="filter pad">
            <div class="filter_left">
                Filter 
                <svg xmlns="http://www.w3.org/2000/svg"  class="icon-filter"><symbol id="icon-filter" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 19 18">
                        <path d="M8.75 17.75V12.25H10.25V14.25H18.25V15.75H10.25V17.75H8.75ZM0.75 15.75V14.25H6.25V15.75H0.75ZM4.75 11.75V9.75H0.75V8.25H4.75V6.25H6.25V11.75H4.75ZM8.75 9.75V8.25H18.25V9.75H8.75ZM12.75 5.75V0.25H14.25V2.25H18.25V3.75H14.25V5.75H12.75ZM0.75 3.75V2.25H10.25V3.75H0.75Z" fill="currentColor"></path>
                    </symbol><use href="#icon-filter"></use>
                </svg>
            </div>
            <div class="filter_right">
                Sort 
                <svg xmlns="http://www.w3.org/2000/svg" class="icon_sort"><symbol id="icon-sort" fill="none" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 12">
                        <path d="M0.5 11.5V10H5.875V11.5H0.5ZM0.5 6.75V5.25H11.6827V6.75H0.5ZM0.5 2V0.5H17.5V2H0.5Z" fill="currentColor">
                        </path>
                    </symbol><use href="#icon-sort"></use>
                </svg>
                <div class="sort_dropdown">
                    <div class="filter_sort-option">
                        <label class="filter_sort-label">
                            <input type="radio" name="sort" value="bestseller" />
                            <span>Best sellers</span>
                        </label>
                        <label class="filter_sort-label">
                            <input type="radio" name="sort" value="bestseller" />
                            <span>Best sellers</span>
                        </label>
                        <label class="filter_sort-label">
                            <input type="radio" name="sort" value="bestseller" />
                            <span>Best sellers</span>
                        </label>
                        <label class="filter_sort-label">
                            <input type="radio" name="sort" value="bestseller" />
                            <span>Best sellers</span>
                        </label>
                        <label class="filter_sort-label">
                            <input type="radio" name="sort" value="bestseller" />
                            <span>Best sellers</span>
                        </label>
                        
                    </div>
                </div>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" class="icon_home close_icon_product hidden"><symbol id="icon-close" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"></path>
                </symbol><use href="#icon-close"></use>
            </svg>
        </div>
        <ol class="product_filter_list pad">
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Availability
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
                

                <div class="dropdown_filter">
                    <ul>
                        <li>In stock</li>
                        <li>Out of stock</li>
                    </ul>
                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Price
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>

                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Category
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Room
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Color
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Material
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
            </li>
            <li class="product_filter_list-item">
                <div class="product_filter_list-item-title">
                    Collection
                    <svg xmlns="http://www.w3.org/2000/svg" class="icon_down"><symbol id="icon-angle-down" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path d="M12 15.0538L6.34625 9.4L7.4 8.34625L12 12.9463L16.6 8.34625L17.6538 9.4L12 15.0538Z" fill="black"></path>
                        </symbol><use href="#icon-angle-down"></use>
                    </svg>
                </div>
            </li>
        </ol>
    </div>
    <div class="quantity-product__status-col pad">
        <div class="quantity-product">
            <span>55 Products</span>
        </div>
        <div class="status-col">
            <span class="status-col-title">View:</span>
            <div class="status-col1-2">
                <button class="btn-col1 add-btn-col">
                    <svg xmlns="http://www.w3.org/2000/svg" class="product-col1" viewBox="0 0 16 16" fill="none">
                        <path class="fill-light-border-30" d="M3.5372 13.6667C3.20042 13.6667 2.91536 13.55 2.68203 13.3167C2.4487 13.0833 2.33203 12.7983 2.33203 12.4615V3.53849C2.33203 3.20172 2.4487 2.91666 2.68203 2.68333C2.91536 2.44999 3.20042 2.33333 3.5372 2.33333H12.4602C12.797 2.33333 13.082 2.44999 13.3154 2.68333C13.5487 2.91666 13.6654 3.20172 13.6654 3.53849V12.4615C13.6654 12.7983 13.5487 13.0833 13.3154 13.3167C13.082 13.55 12.797 13.6667 12.4602 13.6667H3.5372Z"></path>
                    </svg>
                </button>
                <button class="btn-col2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="product-col2" viewBox="0 0 16 16" fill="none">
                        <path  d="M10.0006 13.6667C9.66378 13.6667 9.37873 13.55 9.1454 13.3167C8.91206 13.0833 8.7954 12.7983 8.7954 12.4615V3.53849C8.7954 3.20172 8.91206 2.91666 9.1454 2.68333C9.37873 2.44999 9.66378 2.33333 10.0006 2.33333H12.2569C12.5937 2.33333 12.8787 2.44999 13.1121 2.68333C13.3454 2.91666 13.4621 3.20172 13.4621 3.53849V12.4615C13.4621 12.7983 13.3454 13.0833 13.1121 13.3167C12.8787 13.55 12.5937 13.6667 12.2569 13.6667H10.0006ZM3.74423 13.6667C3.40745 13.6667 3.1224 13.55 2.88906 13.3167C2.65573 13.0833 2.53906 12.7983 2.53906 12.4615V3.53849C2.53906 3.20172 2.65573 2.91666 2.88906 2.68333C3.1224 2.44999 3.40745 2.33333 3.74423 2.33333H6.00056C6.33734 2.33333 6.6224 2.44999 6.85573 2.68333C7.08906 2.91666 7.20573 3.20172 7.20573 3.53849V12.4615C7.20573 12.7983 7.08906 13.0833 6.85573 13.3167C6.6224 13.55 6.33734 13.6667 6.00056 13.6667H3.74423Z"></path>
                    </svg>
                </button>
            </div>
            
        </div>
        <div class="sort_by">
            <span>Sort by: <span >Recommended</span></span>
            
        </div>
    </div>
    <div class="product_list_col-1 pad" id="productList">
        <div class="product_item ">
            <div class="product_item-img discout-new">
                <div class="product_item-img-container">
                    <img src="/NT/public/images/product/1.webp" class="product-img" alt="">
                    <img src="/NT/public/images/product/1-1.webp" class="product-img" alt="">
                </div>
                <div class="btn-left-right">
                    <button class="btn-arrow btn-product-left">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <polyline points="15 18 9 12 15 6"></polyline>
                        </svg>
                    </button>
                    <button class="btn-arrow btn-product-right">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="product_item-content">
                <div class="product_item-content-name-price">
                    <h3 class="product_item-content-name">Product Name</h3>
                    <p class="product_item-content-price">$100.00</p>
                </div>
                <div class="product_item-content-link-detail">
                    <a href="?controller=product&action=detail&id=1" >
                        <span class="product_item-content-link">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon-add"><symbol id="icon-plus" fill="none" viewBox="0 0 16 16">
                                <path d="M7.50033 8.5H3.66699V7.5H7.50033V3.66667H8.50033V7.5H12.3337V8.5H8.50033V12.3333H7.50033V8.5Z" fill="currentColor"></path>
                                </symbol><use href="#icon-plus"></use>
                            </svg>
                            More option available
                        </span>
                    </a>
                </div>
            </div>
        </div>
        <div class="product_item discout-10">
            <div class="product_item-img">
                <div class="product_item-img-container">
                    <img src="/NT/public/images/product/1.webp" class="product-img" alt="">
                    <img src="/NT/public/images/product/1-1.webp" class="product-img" alt="">
                    
                    
                </div>
                <div class="btn-left-right">
                    <button class="btn-arrow btn-product-left">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <polyline points="15 18 9 12 15 6"></polyline>
                        </svg>
                    </button>
                    <button class="btn-arrow btn-product-right">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <polyline points="9 18 15 12 9 6"></polyline>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="product_item-content">
                <div class="product_item-content-name-price">
                    <h3 class="product_item-content-name">Product Name</h3>
                    <p class="product_item-content-price">$100.00</p>
                </div>
                <div class="product_item-content-link-detail">
                    <a href="?controller=product&action=detail&id=1" >
                        <span class="product_item-content-link">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon-add"><symbol id="icon-plus" fill="none" viewBox="0 0 16 16">
                                <path d="M7.50033 8.5H3.66699V7.5H7.50033V3.66667H8.50033V7.5H12.3337V8.5H8.50033V12.3333H7.50033V8.5Z" fill="currentColor"></path>
                                </symbol><use href="#icon-plus"></use>
                            </svg>
                            More option available
                        </span>
                    </a>
                </div>
            </div>
        </div>
        <!-- <div class="product_item product_item-big">
            <div class="product_item-img">
                <div class="product_item-img-container">
                    <img src="/NT/public/images/product/2.webp" class="product-img" alt="">
                    <img src="/NT/public/images/product/2.webp" class="product-img" alt="">
                </div>
            </div>
            <div class="product_item-content">
                <div class="product_item-content-name-price">
                    <h3 class="product_item-content-name">Product Name</h3>
                    <p class="product_item-content-price">$100.00</p>
                </div>
                <div class="product_item-content-link-detail">
                    <a href="?controller=product&action=detail&id=1" >
                        <span class="product_item-content-link">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon-add"><symbol id="icon-plus" fill="none" viewBox="0 0 16 16">
                                <path d="M7.50033 8.5H3.66699V7.5H7.50033V3.66667H8.50033V7.5H12.3337V8.5H8.50033V12.3333H7.50033V8.5Z" fill="currentColor"></path>
                                </symbol><use href="#icon-plus"></use>
                            </svg>
                            More option available
                        </span>
                    </a>
                </div>
            </div>
        </div> -->
        
    </div>
    <div id="loading_spinner">
        <img src="/NT/public/images/gif/loading.gif" alt="Đang tải..." class="loading_spinner-img">
    </div>
    <div class="product pad ">
        <div class="usp ">
            <section class="usp-section">
                <div class="usp-item">
                    <div class="usp-item-left">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon-usp light "><symbol id="icon-light" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M29.7294 11.822L30.886 11.3479V11.3479L29.7294 11.822ZM30.5304 15.7828L29.2804 15.7958L29.2933 17.0328H30.5304V15.7828ZM27.4465 8.36918L28.3351 7.48999L28.3351 7.48999L27.4465 8.36918ZM15.9701 6.0621L16.4528 7.21514L16.4528 7.21514L15.9701 6.0621ZM12.5537 8.36919L11.6651 7.48999L11.6651 7.48999L12.5537 8.36919ZM10.2709 11.822L9.11423 11.3479H9.11423L10.2709 11.822ZM9.46982 15.7828V17.0328H10.7069L10.7198 15.7958L9.46982 15.7828ZM9.46924 15.7828V14.5328H8.21924V15.7828H9.46924ZM9.46924 19.1437H8.21924V20.3937H9.46924V19.1437ZM19.3279 19.1437H20.5779V17.8937H19.3279V19.1437ZM19.3279 23.2517L19.5644 24.4791L20.5779 24.2838V23.2517H19.3279ZM18.5259 23.4924L19.0042 24.6472H19.0042L18.5259 23.4924ZM16.2825 25.7357L15.1277 25.2573L16.2825 25.7357ZM15.9685 27.2099V28.4599H17.1852L17.2181 27.2437L15.9685 27.2099ZM15.967 27.2099V25.9599H14.717V27.2099H15.967ZM15.967 35.5002H14.717V36.7502H15.967V35.5002ZM24.2573 35.5002V36.7502H25.5073V35.5002H24.2573ZM24.2573 27.2099H25.5073V25.9599H24.2573V27.2099ZM24.2557 27.2099L23.0062 27.2437L23.0391 28.4599H24.2557V27.2099ZM23.9417 25.7357L22.7869 26.214L23.9417 25.7357ZM21.6984 23.4924L21.22 24.6472L21.22 24.6472L21.6984 23.4924ZM20.6723 23.2148H19.4223V24.3059L20.5034 24.4534L20.6723 23.2148ZM20.6723 19.1437V17.8937H19.4223V19.1437H20.6723ZM30.531 19.1437V20.3937H31.781V19.1437H30.531ZM30.531 15.7828H31.781V14.5328H30.531V15.7828ZM28.5727 12.296C29.0277 13.4061 29.2679 14.5942 29.2804 15.7958L31.7803 15.7698C31.7645 14.2524 31.4612 12.7513 30.886 11.3479L28.5727 12.296ZM26.558 9.24838C27.4204 10.12 28.1053 11.1555 28.5727 12.296L30.886 11.3479C30.295 9.90599 29.4284 8.59494 28.3351 7.48999L26.558 9.24838ZM23.5474 7.21514C24.6725 7.68611 25.6956 8.37679 26.558 9.24838L28.3351 7.48999C27.2417 6.38501 25.943 5.50779 24.5128 4.90905L23.5474 7.21514ZM20.0001 6.50195C21.217 6.50195 22.4224 6.74418 23.5474 7.21514L24.5128 4.90905C23.0825 4.3103 21.549 4.00195 20.0001 4.00195V6.50195ZM16.4528 7.21514C17.5778 6.74418 18.7832 6.50195 20.0001 6.50195V4.00195C18.4512 4.00195 16.9177 4.3103 15.4874 4.90905L16.4528 7.21514ZM13.4422 9.24838C14.3046 8.37679 15.3277 7.68611 16.4528 7.21514L15.4874 4.90905C14.0572 5.50779 12.7585 6.38502 11.6651 7.48999L13.4422 9.24838ZM11.4275 12.296C11.8949 11.1555 12.5798 10.12 13.4422 9.24838L11.6651 7.48999C10.5718 8.59494 9.70522 9.90599 9.11423 11.3479L11.4275 12.296ZM10.7198 15.7958C10.7323 14.5942 10.9725 13.4061 11.4275 12.296L9.11423 11.3479C8.53904 12.7513 8.23569 14.2524 8.21989 15.7698L10.7198 15.7958ZM9.46924 17.0328H9.46982V14.5328H9.46924V17.0328ZM10.7192 15.8948V15.7828H8.21924V15.8948H10.7192ZM10.7192 19.1437V15.8948H8.21924V19.1437H10.7192ZM19.3279 17.8937H9.46924V20.3937H19.3279V17.8937ZM20.5779 23.2517V19.1437H18.0779V23.2517H20.5779ZM19.0042 24.6472C19.1852 24.5722 19.3729 24.516 19.5644 24.4791L19.0914 22.0243C18.7346 22.093 18.3848 22.1978 18.0475 22.3375L19.0042 24.6472ZM18.065 25.2748C18.3338 25.006 18.653 24.7927 19.0042 24.6472L18.0475 22.3375C17.3929 22.6086 16.7982 23.006 16.2972 23.507L18.065 25.2748ZM17.4374 26.214C17.5829 25.8628 17.7961 25.5436 18.065 25.2748L16.2972 23.507C15.7962 24.008 15.3988 24.6028 15.1277 25.2573L17.4374 26.214ZM17.2181 27.2437C17.2276 26.8902 17.3019 26.5412 17.4374 26.214L15.1277 25.2573C14.8752 25.867 14.7368 26.5174 14.719 27.1761L17.2181 27.2437ZM15.967 28.4599H15.9685V25.9599H15.967V28.4599ZM17.217 27.322V27.2099H14.717V27.322H17.217ZM17.217 35.5002V27.322H14.717V35.5002H17.217ZM24.2573 34.2502H15.967V36.7502H24.2573V34.2502ZM23.0073 27.322V35.5002H25.5073V27.322H23.0073ZM23.0073 27.2099V27.322H25.5073V27.2099H23.0073ZM24.2557 28.4599H24.2573V25.9599H24.2557V28.4599ZM22.7869 26.214C22.9224 26.5412 22.9966 26.8902 23.0062 27.2437L25.5053 27.1761C25.4875 26.5174 25.3491 25.867 25.0966 25.2573L22.7869 26.214ZM22.1593 25.2748C22.4281 25.5436 22.6414 25.8628 22.7869 26.214L25.0966 25.2573C24.8254 24.6028 24.428 24.008 23.9271 23.507L22.1593 25.2748ZM21.22 24.6472C21.5713 24.7927 21.8905 25.006 22.1593 25.2748L23.9271 23.507C23.4261 23.006 22.8313 22.6086 22.1768 22.3375L21.22 24.6472ZM20.5034 24.4534C20.7491 24.4869 20.99 24.5519 21.22 24.6472L22.1768 22.3375C21.748 22.1599 21.299 22.0388 20.8412 21.9763L20.5034 24.4534ZM19.4223 19.1437V23.2148H21.9223V19.1437H19.4223ZM30.531 17.8937H20.6723V20.3937H30.531V17.8937ZM29.281 15.8948V19.1437H31.781V15.8948H29.281ZM29.281 15.7828V15.8948H31.781V15.7828H29.281ZM30.5304 17.0328H30.531V14.5328H30.5304V17.0328Z" fill="black"></path>
                            </symbol><use href="#icon-light"></use>
                        </svg>
                    </div>
                    <div class="usp-item-right">
                        <h4>THIẾT KẾ ĐỘC BẢN</h4>
                        <p>Khám phá các sản phẩm thiết kế giới hạn, độc quyền và đậm chất nghệ thuật.</p>
                    </div>    
                </div>

                <div class="usp-item">
                    <div class="usp-item-left">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon-usp shield "><symbol id="icon-shield" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M18.2498 26.4167L27.6665 17L25.2915 14.625L18.2498 21.6667L14.7498 18.1667L12.3748 20.5417L18.2498 26.4167ZM19.9998 37.1667C16.1387 36.1945 12.9512 33.9792 10.4373 30.5209C7.92345 27.0625 6.6665 23.2223 6.6665 19V8.83337L19.9998 3.83337L33.3332 8.83337V19C33.3332 23.2223 32.0762 27.0625 29.5623 30.5209C27.0484 33.9792 23.8609 36.1945 19.9998 37.1667ZM19.9998 33.6667C22.8887 32.75 25.2776 30.9167 27.1665 28.1667C29.0554 25.4167 29.9998 22.3612 29.9998 19V11.125L19.9998 7.37504L9.99984 11.125V19C9.99984 22.3612 10.9443 25.4167 12.8332 28.1667C14.7221 30.9167 17.1109 32.75 19.9998 33.6667Z" fill="black"></path>
                            </symbol>
                            <use href="#icon-shield"></use>
                        </svg>
                    </div>
                    <div class="usp-item-right">
                        <h4>BẢO HÀNH KÉO DÀI</h4>
                        <p>Hưởng thêm 1 năm bảo hành mở rộng cho mọi đơn hàng.</p>
                    </div>
                </div>

                <div class="usp-item">
                    <div class="usp-item-left">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon-usp lock "><symbol id="icon-lock" viewBox="0 0 40 41" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M10.5129 36.3335C9.68181 36.3335 8.97181 36.0392 8.38292 35.4506C7.79431 34.8617 7.5 34.1517 7.5 33.3206V17.6797C7.5 16.8486 7.79431 16.1386 8.38292 15.5497C8.97181 14.9611 9.68181 14.6668 10.5129 14.6668H12.5V11.3335C12.5 9.25239 13.2297 7.48211 14.6892 6.02266C16.1486 4.56322 17.9189 3.8335 20 3.8335C22.0811 3.8335 23.8514 4.56322 25.3108 6.02266C26.7703 7.48211 27.5 9.25239 27.5 11.3335V14.6668H29.4871C30.3182 14.6668 31.0282 14.9611 31.6171 15.5497C32.2057 16.1386 32.5 16.8486 32.5 17.6797V33.3206C32.5 34.1517 32.2057 34.8617 31.6171 35.4506C31.0282 36.0392 30.3182 36.3335 29.4871 36.3335H10.5129ZM10.5129 33.8335H29.4871C29.6368 33.8335 29.7597 33.7854 29.8558 33.6893C29.9519 33.5932 30 33.4703 30 33.3206V17.6797C30 17.53 29.9519 17.4071 29.8558 17.311C29.7597 17.2149 29.6368 17.1668 29.4871 17.1668H10.5129C10.3632 17.1668 10.2403 17.2149 10.1442 17.311C10.0481 17.4071 10 17.53 10 17.6797V33.3206C10 33.4703 10.0481 33.5932 10.1442 33.6893C10.2403 33.7854 10.3632 33.8335 10.5129 33.8335ZM20 28.4168C20.8097 28.4168 21.4983 28.1332 22.0658 27.566C22.6331 26.9985 22.9167 26.3099 22.9167 25.5002C22.9167 24.6904 22.6331 24.0018 22.0658 23.4343C21.4983 22.8671 20.8097 22.5835 20 22.5835C19.1903 22.5835 18.5017 22.8671 17.9342 23.4343C17.3669 24.0018 17.0833 24.6904 17.0833 25.5002C17.0833 26.3099 17.3669 26.9985 17.9342 27.566C18.5017 28.1332 19.1903 28.4168 20 28.4168ZM15 14.6668H25V11.3335C25 9.94461 24.5139 8.76405 23.5417 7.79183C22.5694 6.81961 21.3889 6.3335 20 6.3335C18.6111 6.3335 17.4306 6.81961 16.4583 7.79183C15.4861 8.76405 15 9.94461 15 11.3335V14.6668Z" fill="black"></path>
                            </symbol><use href="#icon-lock"></use>
                        </svg>
                    </div>
                    <div class="usp-item-right">
                        <h4>GIAO HÀNG MIỄN PHÍ</h4>
                        <p>Miễn phí vận chuyển toàn quốc cho tất cả đơn hàng online.</p>
                    </div>
                </div>

                <div class="usp-item">
                    <div class="usp-item-left">
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon-usp package "><symbol id="icon-package" viewBox="0 0 30 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M13.75 29.1153V17.2178L3.33333 11.1858V22.7949C3.33333 22.8802 3.35472 22.9603 3.39749 23.0353C3.44027 23.1101 3.5043 23.1741 3.58958 23.2274L13.75 29.1153ZM16.25 29.1153L26.4104 23.2274C26.4957 23.1741 26.5597 23.1101 26.6025 23.0353C26.6453 22.9603 26.6667 22.8802 26.6667 22.7949V11.1858L16.25 17.2178V29.1153ZM13.4937 31.8428L2.34 25.4228C1.86555 25.1495 1.49583 24.7842 1.23083 24.327C0.965828 23.8695 0.833328 23.3673 0.833328 22.8203V10.1795C0.833328 9.63255 0.965828 9.13033 1.23083 8.67283C1.49583 8.21561 1.86555 7.85033 2.34 7.577L13.4937 1.157C13.9679 0.883389 14.47 0.746582 15 0.746582C15.53 0.746582 16.0321 0.883389 16.5062 1.157L27.66 7.577C28.1344 7.85033 28.5042 8.21561 28.7692 8.67283C29.0342 9.13033 29.1667 9.63255 29.1667 10.1795V22.8203C29.1667 23.3673 29.0342 23.8695 28.7692 24.327C28.5042 24.7842 28.1344 25.1495 27.66 25.4228L16.5062 31.8428C16.0321 32.1164 15.53 32.2532 15 32.2532C14.47 32.2532 13.9679 32.1164 13.4937 31.8428ZM21.4262 11.3332L25.2917 9.11533L15.2562 3.31075C15.171 3.25742 15.0856 3.23075 15 3.23075C14.9144 3.23075 14.829 3.25742 14.7437 3.31075L11.125 5.39408L21.4262 11.3332ZM15 15.0641L18.875 12.8203L8.58333 6.87158L4.70833 9.11533L15 15.0641Z" fill="black"></path>
                            </symbol><use href="#icon-package"></use>
                        </svg>
                    </div>
                    <div class="usp-item-right">
                        <h4>ĐỔI TRẢ DỄ DÀNG</h4>
                        <p>Không hài lòng? Đổi trả nhanh gọn trong vòng 30 ngày.</p>
                    </div>
                </div>
            </section>
        </div>
    </div>
</div>
